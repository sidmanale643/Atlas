...
import { REGION_ANCHORS } from "../src/shared/region-anchors.js";
import {
  calculateMemoryPosition,
  calculateRegionPosition,
  createMemoryNodeState,
  getDominantRegion,
} from "../src/shared/memory-placement.js";
...
test("centers balanced hippocampal memories between both hemispheres", () => {
  const position = calculateMemoryPosition("mem_balanced", [{
    region: "hippocampus",
    weight: 1,
    hemispheres: { left: 0.5, right: 0.5 },
  }]);
  const expected = calculateRegionPosition(
    "mem_balanced",
    "hippocampus",
    REGION_ANCHORS.hippocampus.position,
  );

  assert.deepEqual(position, expected);
});

test("shifts hippocampal placement toward the more active hemisphere", () => {
  const leftBiased = calculateMemoryPosition("mem_shift", [{
    region: "hippocampus",
    weight: 1,
    hemispheres: { left: 0.65, right: 0.35 },
  }]);
  const rightBiased = calculateMemoryPosition("mem_shift", [{
    region: "hippocampus",
    weight: 1,
    hemispheres: { left: 0.35, right: 0.65 },
  }]);

  assert.ok(leftBiased[0] < rightBiased[0]);
});

test("preserves bilateral footprints while keeping one canonical core", () => {
  const state = createMemoryNodeState([{
    id: "mem_bilateral",
    regions: [{
      region: "hippocampus",
      weight: 1,
      hemispheres: { left: 0.45, right: 0.55 },
    }],
  }]).get("mem_bilateral");

  assert.deepEqual(state.core.hemispheres, { left: 0.45, right: 0.55 });
  assert.deepEqual(
    state.activations[0].hemispheres,
